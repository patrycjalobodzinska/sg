...
const eyebrow = (t?: string) => (
  <div style={{ color: "#1F52B8", fontSize: 13, fontWeight: 600, letterSpacing: ".1em", textTransform: "uppercase", marginBottom: 14 }}>{t}</div>
);
...
export default function InvestorsPage({ lang, doc }: { lang: Locale; doc: InvDoc }) {
  ...
  return (
    <div style={{ background: "#EFF1F5", color: "#14161C", overflowX: "hidden" }}>
      ...
      {/* HERO (dark) */}
      <header style={{ position: "relative", overflow: "hidden", background: "#0B1220", color: "#fff", borderRadius: "0 0 clamp(24px,3vw,36px) clamp(24px,3vw,36px)", padding: "clamp(88px,10vw,116px) clamp(20px,5vw,64px) clamp(48px,6vw,88px)" }}>
      ...
      {/* MISSION BAND */}
      <section style={{ background: "#0E1526", color: "#fff", borderRadius: "clamp(24px,3vw,36px)", margin: "clamp(8px,2vw,20px) clamp(12px,2vw,24px)", padding: "clamp(32px,4vw,60px) clamp(24px,5vw,64px)" }}>
        ...
        <div data-inv2="1" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "clamp(28px,4vw,56px)", alignItems: "center" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            {(platform.features ?? []).map((f) => (
              <div key={f} style={{ background: "#fff", border: "1px solid rgba(24,30,48,.06)", borderRadius: 12, padding: "18px 20px", fontSize: 15, fontWeight: 500, color: "#14161C", boxShadow: "0 10px 30px rgba(20,26,48,.04)", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          {benefits.map((b, i) => (
            <div key={b.tag || i} style={{ background: "#fff", border: "1px solid rgba(24,30,48,.06)", borderRadius: 12, padding: "clamp(26px,3vw,34px)", boxShadow: "0 12px 40px rgba(20,26,48,.05)" }}>
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "flex-start", color: "#14161C", fontSize: 14.5, fontWeight: 600, lineHeight: 1.45 }}>
                <span style={{ color: "#1F52B8", flex: "none" }}>→</span>{b.benefit}
              </div>
            </div>
          ))}
        </div>
      ...
      <section style={{ maxWidth: 1440, margin: "0 auto", padding: "clamp(28px,4vw,54px) clamp(20px,5vw,64px)" }}>
        <div style={{ maxWidth: 760, marginBottom: 30 }}>
          ...
          <p style={{ margin: "16px 0 0", color: "#5A6275", fontSize: 16.5, lineHeight: 1.55 }}>{doc?.marketSubtitle}</p>
        </div>
        <div data-invmarket="1" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14 }}>
          {market.map((m) => (
            <div key={m} style={{ background: "#fff", border: "1px solid rgba(24,30,48,.06)", borderRadius: 12, padding: "20px 22px", fontSize: 15.5, fontWeight: 500, color: "#14161C", boxShadow: "0 10px 30px rgba(20,26,48,.04)" }}>{m}</div>
          ))}
        </div>
      </section>
      ...
      <section style={{ background: "#0E1526", color: "#fff", borderRadius: "clamp(24px,3vw,36px)", margin: "clamp(18px,3vw,34px) clamp(12px,2vw,24px)", padding: "clamp(34px,4vw,62px) clamp(24px,5vw,64px)", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 1240, margin: "0 auto", position: "relative", zIndex: 1 }}>
          ...
          <div data-invmarket="1" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14 }}>
            {thesis.map((x) => (
              <div key={x} style={{ background: "rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.1)", borderRadius: 12, padding: "18px 20px", fontSize: 15, lineHeight: 1.45, display: "flex", gap: 10 }}>
