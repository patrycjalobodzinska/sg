    ...
    <div style={{ background: "#EFF1F5", color: "#14161C", overflowX: "hidden" }}>
      ...
      <header style={{ maxWidth: 1440, margin: "0 auto", padding: "clamp(72px,7vw,92px) clamp(20px,5vw,64px) clamp(12px,2vw,22px)" }}>
        <h1 style={{ margin: 0, fontSize: "clamp(38px,6vw,72px)", lineHeight: 1.02, letterSpacing: "-.025em" }}>
          {t.heroTitle} <span style={{ color: "#2E6BE6", fontWeight: 500 }}>{t.heroAccent}</span>
        </h1>
        ...
        <section style={{ maxWidth: 1440, margin: "0 auto", padding: "clamp(14px,2vw,24px) clamp(20px,5vw,64px)" }}>
          <a href={href(featured.slug)} data-newsfeat="1" style={{ display: "grid", gridTemplateColumns: "1.15fr 1fr", gap: "clamp(24px,4vw,48px)", alignItems: "stretch", background: "#fff", border: "1px solid rgba(24,30,48,.06)", borderRadius: 18, overflow: "hidden", boxShadow: "0 18px 52px rgba(20,26,48,.06)", color: "inherit" }}>
            ...
            <div style={{ padding: "clamp(28px,3.5vw,48px)", display: "flex", flexDirection: "column", justifyContent: "center" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16, color: "#5A6275", fontSize: 13.5 }}>
                <span style={{ fontSize: 12, color: "#1F52B8", background: "#E9F0FC", padding: "5px 12px", borderRadius: 999, fontWeight: 600 }}>{featured.category || t.categoryDefault}</span>
              ...
              <span style={{ display: "inline-flex", alignItems: "center", gap: 7, marginTop: 22, color: "#1F52B8", fontSize: 15.5, fontWeight: 600 }}>{t.readMore} <ArrowUpRight /></span>
            </div>
          </a>
        </section>
      )}
            ...
            {items.map((it) => (
              <a key={it._id} href={href(it.slug)} style={{ background: "#fff", border: "1px solid rgba(24,30,48,.06)", borderRadius: 18, overflow: "hidden", boxShadow: "0 12px 40px rgba(20,26,48,.05)", display: "flex", flexDirection: "column", color: "inherit" }}>
                ...
                <div style={{ padding: 24, display: "flex", flexDirection: "column", flex: 1 }}>
                  <div style={{ color: "#5A6275", fontSize: 13, marginBottom: 12 }}>{formatNewsDate(it.date, lang)}</div>
                  <h3 style={{ margin: "0 0 10px", fontSize: 18.5, fontWeight: 600, letterSpacing: "-.01em", lineHeight: 1.22, ...clamp(3) }}>{it.title}</h3>
                  {it.excerpt && <p style={{ margin: 0, color: "#5A6275", fontSize: 14.5, lineHeight: 1.55, ...clamp(4) }}>{it.excerpt}</p>}
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 6, marginTop: 16, color: "#1F52B8", fontSize: 14, fontWeight: 600 }}>{t.readMore} <ArrowUpRight /></span>
                </div>
              </a>
            ))}
          </NewsCarousel>
      ...
      <section style={{ maxWidth: 1440, margin: "0 auto", padding: "clamp(8px,2vw,20px) clamp(20px,5vw,64px) clamp(32px,5vw,64px)" }}>
        <div style={{ background: "#0E1526", color: "#fff", borderRadius: "clamp(24px,3vw,36px)", padding: "clamp(32px,4vw,56px)", display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: 20 }}>
          ...
          <a href={contactHref} className="sheen" style={{ display: "inline-flex", alignItems: "center", gap: 9, background: "#2E6BE6", color: "#fff", padding: "16px 28px", borderRadius: 999, fontWeight: 600, fontSize: 16, whiteSpace: "nowrap" }}>{t.ctaButton} <ArrowUpRight /></a>
        </div>
      </section>
